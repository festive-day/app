 ...
 *    - Dynamic expression in instance attribute
 *    - Empty/null handling
 *
 ...
 *    - Dynamic expressions work with specialized types
 *
 * ✅ String Property (specialized: array - Loop Props)
 *    - Loop prop resolves wp-query loop handler
 *    - Loop prop resolves wp-users loop handler
 *    - Loop prop resolves wp-terms loop handler
 *    - Loop prop resolves json loop handler
 *    - Loop prop by database key (loop ID)
 *    - Loop prop by key property (find_loop_by_key)
 *    - Loop prop with params: "posts(status=draft)"
 *    - Loop prop from context expression: {this.relatedPosts}
 *    - Loop prop fallback to JSON parsing
 *    - Loop prop fallback to comma-separated parsing
 *    - Already array value returns as-is
 *
 * ✅ Number Property
 *    - Default numeric value
 *    - Instance attribute override
 *    - String to number conversion
 *    - Dynamic expression resolves to number
 ...
 *    - Default true value
 *    - Default false value
 *    - Instance attribute "true" string
 *    - Instance attribute "false" string
 *    - Instance attribute boolean true
 *    - Instance attribute boolean false
 *    - Dynamic expression resolves to boolean
 *    - Used in HTML attributes (data-test="{props.enabled}")
 *    - Used in ConditionBlock conditions
 *
 * ✅ Object Property
 *    - Default object value
 *    - Instance attribute JSON string
 *    - Instance attribute array
 *    - Dynamic expression resolves to object
 *    - Nested object access
 *
 * ✅ Array Property (primitive array, not specialized loop)
 *    - Default array value
 *    - Instance attribute JSON string
 *    - Instance attribute comma-separated string
 *    - Instance attribute array
 *    - Dynamic expression resolves to array
 *
...
class ComponentPropertyResolverTest extends WP_UnitTestCase {
	...
	public function test_string_property_default_literal() {
		...
		$resolved = ComponentPropertyResolver::resolve_properties( $property_definitions, $instance_attributes, $context );
	...
	/**
	 * Test string property with dynamic expression in instance attribute
	 */
	public function test_string_property_dynamic_instance() {
		$property_definitions = array(
			array(
				'key'  => 'title',
				'name' => 'Title',
	...
	/**
	 * Test loop prop resolves wp-query loop handler
	 */
	public function test_loop_prop_resolves_wp_query_handler() {
		...
		// wp-query handler should return array of posts
		$this->assertNotEmpty( $resolved['loop'] );
	}

	/**
	 * Test loop prop resolves wp-users loop handler
	 */
	...
	/**
	 * Test loop prop resolves wp-terms loop handler
	 */
	...
	/**
	 * Test loop prop resolves json loop handler
	 */
	...
	/**
	 * Test loop prop by key property (find_loop_by_key)
	 */
	...
	/**
	 * Test loop prop from context expression
	 */
	public function test_loop_prop_from_context_expression() {
		$property_definitions = array(
			array(
				'key'  => 'loop',
				'name' => 'Loop',
		...
		$this->assertIsArray( $resolved['loop'] );
		$this->assertCount( 2, $resolved['loop'] );
		$this->assertEquals( 'Post 1', $resolved['loop'][0]['title'] );
	}

	/**
	 * Test loop prop fallback to JSON parsing
	 */
	public function test_loop_prop_fallback_json() {
		...
		$this->assertIsArray( $resolved['loop'] );
		$this->assertCount( 3, $resolved['loop'] );
		$this->assertEquals( 'item1', $resolved['loop'][0] );
	}

	/**
	 * Test loop prop fallback to comma-separated parsing
	 */
	public function test_loop_prop_fallback_comma_separated() {
		...
		$this->assertIsArray( $resolved['loop'] );
		$this->assertCount( 3, $resolved['loop'] );
		$this->assertEquals( 'item1', trim( $resolved['loop'][0] ) );
	}

	/**
	 * Test loop prop with already array value returns as-is
	 */
	public function test_loop_prop_already_array() {
		$property_definitions = array(
			array(
				'key'  => 'loop',
				'name' => 'Loop',
	...
	/**
	 * Test null instance value (overrides default with empty)
	 */
	public function test_null_instance_value() {
		...
		// Null/empty explicitly set should override default (empty is a valid value)
		$this->assertEquals( '', $resolved['title'] );
	}
	...
	/**
	 * Test loop prop with empty string returns empty array
	 */
	public function test_loop_prop_empty_string() {
		...
		$this->assertIsArray( $resolved['loop'] );
		$this->assertEmpty( $resolved['loop'] );
	}
}
