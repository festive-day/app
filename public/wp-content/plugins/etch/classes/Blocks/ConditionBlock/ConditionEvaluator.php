...
class ConditionEvaluator {
	 ...
	 * @param array<string, mixed>|null $context Dynamic context for expression resolution.
	 * @return mixed The evaluated value.
	 */
	public static function evaluate_operand( $operand, ?array $context ) {
		// If it's a nested condition, evaluate it recursively
		if ( is_array( $operand ) && isset( $operand['operator'] ) ) {
			return self::evaluate_condition( $operand, $context ?? array() );
		}
		...
		if ( is_string( $operand ) ) {
			...
			// This handles: quoted strings, numeric strings, boolean strings, path expressions, etc.
			// Pass empty array if context is null (process_expression handles empty context)
			$processed_value = EtchParser::process_expression( $normalized_value, $context ?? array() );
	 ...
	 */
	public static function evaluate_condition( array $condition, ?array $context ): bool {
		...
		if ( isset( $condition['leftHand'] ) ) {
			$left_value = self::evaluate_operand( $condition['leftHand'], $context ?? array() );
		}
		...
		if ( isset( $condition['rightHand'] ) ) {
			$right_value = self::evaluate_operand( $condition['rightHand'], $context ?? array() );
		}
