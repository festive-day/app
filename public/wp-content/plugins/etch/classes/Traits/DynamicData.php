...
trait DynamicData {
	...
	protected function get_dynamic_option_pages_data(): array {
		$options = $this->get_base_option_pages_data();
		$options = $this->add_acf_data( 'option', 0, $options );
		$options = $this->add_metabox_data( 'setting', 0, $options );
		$options = $this->add_jetengine_data( 'option', 0, $options );
	 ...
	 *
	 * @param WP_Post $post The post object.
	 * @return array<string, string> The post meta data.
	 */
