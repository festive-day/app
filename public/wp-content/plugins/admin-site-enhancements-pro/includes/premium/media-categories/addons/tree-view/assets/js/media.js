	...
	function( atts ) {
		...
		( function( $ ) {
			...
			if ( wp.Uploader.queue ) {
				// When term assignment happens during upload of one or more files to a category / sub-category
				...
				if ( complete ) {
					...
					// Do not reload when viewing an item in the grid view, i.e. URL will look like /wp-admin/upload.php?item=11477. 
					if (! searchParams.has('item')) {
						location.reload();
					}
				}				
			} else {
				// When term assignment happens during drag-and-drop of attachment to a category / sub-category
				location.reload();
			}
	...
	function( attachment ) {
		...
		if ( wp.Uploader.queue ) {
			...
			if ( complete ) {
				...
				let searchParams = new URLSearchParams(window.location.search);
				// Do not reload when viewing an item in the grid view, i.e. URL will look like /wp-admin/upload.php?item=11477. 
				// Reload is only needed when viewing the media library with the category tree on it
				if (! searchParams.has('item')) {
					location.reload();
				}
			}
