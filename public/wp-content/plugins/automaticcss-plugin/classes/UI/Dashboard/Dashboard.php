...
class Dashboard {
	...
	private function get_dashboard_enqueue_settings() {
		...
		return array(
			...
			'file_url' => Flag::is_on( 'LOAD_DASHBOARD_FROM_VITE' ) ? 'http://localhost:5173/features/Dashboard/main.js' : ACSS_CLASSES_URL . "{$this->path}/{$filename}",
			'file_version' => Flag::is_on( 'LOAD_DASHBOARD_FROM_VITE' ) ? null : filemtime( ACSS_CLASSES_DIR . "{$this->path}/{$filename}" ),
	...
	public static function add_type_attribute( $tag, $handle, $src ) {
		...
		$defer = Flag::is_on( 'LOAD_DASHBOARD_SCRIPTS_IN_FOOTER' ) ? ' defer' : '';
		$module_and_crossorigin =
			Flag::is_on( 'LOAD_DASHBOARD_SCRIPTS_AS_MODULE' ) || Flag::is_on( 'LOAD_DASHBOARD_FROM_VITE' ) ?
			' type="module" crossorigin' :
			'';
