...
use Automatic_CSS\EDD\EDD_SL_Plugin_Updater;
...
class Plugin_Updater {
	...
	private function get_license_data( $response ) {
		...
		if ( '' === $body ) {
			throw new \Exception( esc_html( __( 'There was a problem retrieving your license information.' ) ) );
		}
		$license_data = json_decode( $body );
		if ( null === $license_data ) {
			throw new \Exception( esc_html( __( 'There was a problem decoding the license information.' ) ) );
		}
		if ( ! isset( $license_data->success ) || ! isset( $license_data->license ) ) {
			throw new \Exception( esc_html( __( 'There was a problem with the license information.' ) ) );
		}
