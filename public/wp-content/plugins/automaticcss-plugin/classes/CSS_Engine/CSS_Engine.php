...
class CSS_Engine {
	...
	public function init() {
		...
		$vars = $this->database_settings->get_vars();
			...
			$this->framework['core'] = new Core( $vars );
			// Initialize platforms.
		if ( Etch::is_active() ) {
			Logger::log( sprintf( '%s: activating Etch module', __METHOD__ ) );
			$this->platforms['etch'] = new Etch( $vars );
		...
		// } // We no longer need to enqueue the Frames CSS file, and there's nothing else to do here.
		return $this;
	}
