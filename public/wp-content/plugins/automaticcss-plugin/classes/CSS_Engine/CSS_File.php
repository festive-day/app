...
class CSS_File {
	...
	public function enqueue_stylesheet() {
		...
		// $this->process_stylesheet(); // TODO: shouldn't need because AssetManager calls do_items().
	}
	...
	public function enqueue_stylesheet_via_add_editor_style( $local_path = '' ) {
		...
		$style_path = ! empty( $local_path ) ? $local_path . $this->filename : $this->file_url;
		Logger::now( 'CSS FILE: enqueuing via add_editor_style ' . $style_path ); // TODO: remove.
	...
	public function enqueue_inline( $handle = '' ) {
		...
		$css = file_get_contents( $this->file_path );
		if ( ! Flag::is_on( 'ENABLE_NEW_ASSET_MANAGEMENT' ) ) {
			$this->enqueue_options['queue']->add_inline_style( $handle, $css );
			return;
		}
		...
		$this->enqueue_options['queue']->add( $handle, '' ); // Breaks if you don't register the handle first.
		$this->enqueue_options['queue']->add_data( $handle, 'inline-css', $css );
	}
	...
	public function __get( $key ) {
		...
		if ( in_array( $key, $allowed_keys ) ) {
			...
			throw new \Exception( esc_html( "Trying to get a not allowed or not set key {$key} on a CSS_File instance" ) );
		}
	}
}
