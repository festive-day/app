...
class Core extends Base {
	 ...
	 * Constructor
	 *
	 * @param array $database_values The database values.
	 */
	public function __construct( $database_values ) {
		...
		$this->tokens_css_file = $this->add_css_file( new CSS_File( 'automaticcss-tokens', 'automatic-tokens.css', 'automatic-tokens.scss' ) );
		...
		$priority = isset( $database_values['option-load-stylesheets-last'] ) && 'on' === $database_values['option-load-stylesheets-last'] ? 20 : 10;
		add_action( 'wp_enqueue_scripts', array( $this, 'enqueue_core_stylesheet' ), $priority ); // ! This > 10 causes the bug.
		add_action( 'wp_enqueue_scripts', array( $this, 'enqueue_inline_styles' ), $priority );
	...
	public function enqueue_inline_styles() {
		// Inline styles are attached to the core CSS file.
		// TODO: Inject Database_Settings as a dependency in the constructor to decouple and make it easier to test.
		$is_option_inline_tokens_on = \Automatic_CSS\Model\Database_Settings::get_instance()->get_var( 'option-inline-tokens' ) === 'on' ?? false;
		if ( $is_option_inline_tokens_on ) {
			$this->tokens_css_file->enqueue_inline( $this->core_css_file->handle );
		}
		$this->custom_css_file->enqueue_inline( $this->core_css_file->handle );
	}
	...
	public function get_frontend_and_preview_stylesheets( $additional_stylesheets ) {
		$core_stylesheets = array(
			...
			array(
				'id' => $this->tokens_css_file->handle,
				'url' => $this->tokens_css_file->file_url,
			),
			array(
	...
	public function get_framework_variables( $values ) {
		if ( Flag::is_on( 'USE_REFACTORED_SCSS_INJECTION_CODE' ) ) {
		...
		// Using the typography.json default root font size generate a wrong value if it was set as 100.
