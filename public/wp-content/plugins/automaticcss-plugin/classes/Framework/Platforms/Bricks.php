...
class Bricks extends Base implements Platform, Builder {
	 ...
	 * Constructor
	 *
	 * @param array $database_values The database settings.
	 */
	public function __construct( $database_values ) {
		...
		if ( is_admin() ) {
			...
			$priority = isset( $database_values['option-load-stylesheets-last'] ) && 'on' === $database_values['option-load-stylesheets-last'] ? 21 : 11;
			add_action( 'wp_enqueue_scripts', array( $this, 'enqueue_builder_assets' ), $priority );
		}
	}
