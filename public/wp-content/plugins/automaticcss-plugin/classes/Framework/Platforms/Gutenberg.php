...
class Gutenberg extends Base implements Platform {
	...
	public function __construct( $is_enabled ) {
		...
		$this->is_load_styling_backend_enabled = $is_enabled;
		...
		$this->editor_iframed_css_file = $this->add_css_file(
			new CSS_File(
		...
		$this->editor_iframed_css_file->set_enabled( $this->is_load_styling_backend_enabled );
		...
		// Move the enqueue actions to the new asset management flag.
	...
	public function enqueue_block_editor_assets() {
		if ( Flag::is_on( 'LOAD_GUTENBERG_STYLES_USING_ENQUEUE_STYLESHEET' ) ) {
			...
			// WordPress don't load correctly when pass full url. For local environment we should pass relative path.
